...
export const listItemsService = async (filters, page, limit) => {
  ...
  const items = await utils.getItems(where, limit, offset, [
    ["created_at", arrange],
  ]);
  ...
  if (filters.category_id) {
    const whereOfCategoryItems = utils.buildWhereForCategoryItems(
      filters,
      items,
    );
    categoryItems = await utils.getItems(whereOfCategoryItems, limit, offset, [
      ["created_at", arrange],
    ]);
