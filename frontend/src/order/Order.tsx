...
import { getOrders } from './getOrders';        // ✅改了: getorders -> getOrders
import { getOrderItems } from './getOrderItems';  // ✅改了: getorderItems -> getOrderItems

export default function Order() {
    // جلب بيانات الأوردر
    let { data } = useQuery({
        queryKey: ['get all orders'],
        queryFn: getOrders  // ✅改了
    });
                    ...
                    {data?.order ? (
                        <div className="space-y-4 md:space-y-5 text-gray-600 text-base md:text-lg">
                            <div className="flex justify-between items-center pb-3 border-b border-gray-100">
                                ...
                                <span className="text-gray-800 font-medium">{data.order.user?.name || 'User'}</span>
                            </div>
